...
describe("PostgresEventHandlerRegistry tests", () => {
    ...
    test("should throw error when committing without obtaining lock", async () => {
        await expect(
            handlerRegistry.commitAndRelease({ [Object.keys(handlers)[0]]: SequenceNumber.create(1) })
    ...
    test("should throw error when committing without sequence number", async () => {
        ...
        await expect(handlerRegistry.commitAndRelease({ [Object.keys(handlers)[0]]: <any>null })).rejects.toThrow(
