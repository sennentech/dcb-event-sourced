...
export const readSqlWithCursor = (criteria: EsQueryCriterion[], options?: EsReadOptions) => {
    const { sql, params } = readSql(criteria, options)
...
export const readCriteriaJoin = (criteria: EsQueryCriterion[], pm: ParamManager, options?: EsReadOptions): string => {
    const criteriaQueries = criteria.map((c) => `
        SELECT 
